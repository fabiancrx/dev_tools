...
// Vendored from devtools package
    ...
    void updateSpacing(DragUpdateDetails dragDetails, int splitterIndex) {
      ...
      setState(() {
        ...
        if (fractionalDelta <= 0.0) {
          final appliedDelta = updateSpacingBeforeSplitterIndex(fractionalDelta);
          updateSpacingAfterSplitterIndex(-appliedDelta);
        } else {
          final appliedDelta = updateSpacingAfterSplitterIndex(-fractionalDelta);
  ...
  double _totalSplitterSize() {
    ...
    if (widget.splitters == null) {
      ...
      for (var splitter in widget.splitters!) {
        totalSize += isHorizontal ? splitter.preferredSize.width : splitter.preferredSize.height;
      }
      return totalSize;
    }
  }
...
class DefaultSplitter extends StatelessWidget {
  const DefaultSplitter({required this.isHorizontal, super.key});
